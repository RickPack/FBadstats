...
#' @return Best or worst performing subgroups depending on tblout parameter, best performers with a table if grphout parameter is YES. Also returns data frame containing the
...
fbadGstats <- function(filerd = "", choosedir = "NO", sumvar = "", filtervar = "", spentlim = 0, minevent = 0, prtrow = 20, tblout = "BOTH", grphout = "YES", ctrstats = "NO") {
    ...
    for (ff in 1:length(allfls)) {
        ...
        # [1] 'Reporting.Starts' 'Reporting.Ends' [3] 'Campaign.Name' 'DMA.Region' [5] 'Amount.Spent.USD.'  'Amount.Spent.Today.USD.'  [7] 'Ends' 'CTR.All.'  [9] 'CTR.Link.Click-Through.Rate.'
        # 'Cost.per.Website.Search.USD.'  [11] 'CPC.Cost.per.Link.Click..USD.'  'Cost.per.Website.Registration.Completed.USD.'  [13] 'Website.Registrations.Completed' 'Link.Clicks' [15] 'Post.Comments'
        # 'Post.Reactions' [17] 'Post.Shares' 'Website.Leads' [19] 'Cost.per.Website.Lead.USD.'  'Leads..Form.'  [21] 'Cost.per.Leads..Form..USD.'  'Cost.per.Page.Like.USD.'
        if (example == 1) {
        ...
        ## failure as.character(dmafb %>% pull(grep('AD.NAME', stringr::str_to_upper(dnams))))
        ...
        dmafb <- dmafb %>% filter(!is.na(CAMPAIGN.NAME)) %>% filter(grepl(filtervar, toupper(CAMPAIGN.NAME)))
        ## for personal use to block university of kentucky ads dmafb <- dmafb %>% filter(!grepl('UNI',toupper(Campaign.Name)))
        
        # if (length(dnams[grepl('AD.SET',toupper(dnams))]) > 0){ if(dnams[grep('REGION',toupper(dnams))] != 'DMA.Region' & ){ DMA.Region <- dmafb[,grep('REGION', stringr::str_to_upper(dnams))]
        # colnames(DMA.Region) <- 'DMA.Region' dmafb <- cbind(DMA.Region, dmafb) }}
        ...
        dmafb2 <- dmafb2 %>% mutate_if(is.numeric, (funs(replace(., is.na(.), 0))))
        dmafb2 <- dmafb2 %>% mutate_if(is.numeric, (funs(replace(., is.infinite(.), 0))))
        
        if (ctrstats %in% c("YES", 1) & length(grep("RATE", dnams3)) >= 1) {
            if (length(grep("CTR..LINK.CLICK.THROUGH.RATE.", dnams3)) == 1) {
                noctr <- 0
                dmafb2$C1 <- as.numeric(dmafb2$CTR..LINK.CLICK.THROUGH.RATE.)
                ctrnam <- "CTR..LINK.CLICK.THROUGH.RATE."
            } else if (length(grep("RATE", dnams3)) == 1) {
                noctr <- 0
                dmafb2$C1 <- dmafb2 %>% select(dnams3[grep("RATE", dnams3)]) %>% pull(1)
        ...
        dmafbgrp <- dmafb2 %>% group_by(!(!grpvar))
        ...
        ####################### Graph section ## Capture the best breakdown groups in sumevtavg
        if (grphoutTF == TRUE) {
            sumevtavg <- arrange(dmastat_all, rnkevt)
